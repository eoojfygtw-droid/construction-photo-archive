...
## 下一步（依優先序）
0. ~~桌機 restart 套用 5-B1 + 5-B2~~ ✅ **已於 6/14 17:02 restart 並實機驗證生效**（6/15 11:04 真群組批次歸檔 log 為證；5-B2/「龍哥來了」同批）。
1. **V0 驗收延長 3 天（6/15–6/17）邊跑邊調**：5 工作天（6/8–6/12）已跑滿、15 筆 _inbox 結算為零系統誤判（全冷啟動沒給依據，見 PROGRESS_LOG 6/14）；使用者 6/15 拍板**再續 3 天觀察後**才宣告 V0 通過進 V1。每日核對照舊（後台 `npm run admin` 篩當天 / `npm run report`）。
...
- [x] **「龍哥來了」彩蛋已套用**（2026-06-14 17:02 restart-bot 連同 5-B1/5-B2 一起生效；訊息含「龍哥來了」→ 回「快跑！🏃💨」）。
