...
9. **驗收期體驗補強**(2026-06-08〜10,邊用邊修):①唯讀 HTML 巡檢日報 `npm run report`;②`/新增工地` 現場可用(座標可選/傳位置設中心);③bot Windows 排程背景常駐+存活監控;④單獨傳定位不再沉默(回覆判定或跳選單,點選只設上下文);⑤**錄音存檔**——語音/音訊當媒體歸檔(`upload_type=voice`,不轉文字,Whisper 仍 V1);⑥**追加合併**——建檔後 10 分鐘內純語音/文字併入上一筆,附「🆕 拆成新筆」反悔;**封單只認 ✅**,選工地/改工地只是歸類不封單;⑦**選單指定工地(manual_pick)也回寫 2 小時上下文**(記回報人、錨收件時間),判不出工地不再每張照片都跳選單;維持固定 2 小時窗不滑動,6/13 連同誤歸觀察一起檢討;⑧**定位判不出選單加「➕ 新增工地」**——剛傳的定位暫存 10 分鐘(`PendingLocationStore`),`/新增工地 代碼 名稱` 自動拿它當中心+設上下文,人在新工地全程不必手打座標。各項詳見 DECISIONS
