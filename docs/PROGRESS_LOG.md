...
## 2026-06-14 桌機（午後續場）：判不出編號修正 + 報告頁 5-A5（lightbox / 錄音 / 列印）🟢
桌機開工後續場。先補早場兩塊的小修，再做後台報告頁。三批分開 commit、各自 typecheck + 10 支 smoke 全綠。

- **判不出回覆修正（`c602c4f`）**：5-B1 批次選單漏掉暫存編號，使用者誤以為「沒給編號」→ 補回 `（INBOX-…-NNN）`。並把 `/新增工地 名稱`（沒先傳定位）的回覆改成明講「現在傳照片不會自動歸、會判不出進待歸檔」，給兩條開自動歸的路。
- **自動歸觸發條件釐清（踩雷對焦）**：使用者實測「`/新增工地 ⟨某新工地名⟩`（自動配碼 A003 成功）後傳照片卻判不出、且第二張流水號是 001 不是 002」。查 DB 還原：①`/新增工地 名稱` **沒先傳定位**時工地無中心、不設 recent_context → 下一張照片五層全 miss、正確進 _inbox（`INBOX-20260614-001`）。②使用者在選單點 A003 → 該筆改歸 A003（record_no **不重編**，仍叫 INBOX-…）+ 播種上下文。③第二張靠 recent_context 自動歸 A003，拿 A003 自己的序列 `A003-20260614-001`。結論：**自動歸只在「裸碼/#代碼、photo_gps(文件)、定位釘、recent_context」其一成立時觸發**；`/新增工地 名稱` 單獨打不會自動歸，要「先傳定位再打名稱」或「第一張帶代碼」。record_no 按 prefix 分序列、改歸不重編＝設計（見 DECISIONS 2026-06-05/06-14）。
- **5-A5 報告頁（`b557440` + 互動升級）**：後台新增 `GET /report`（開會／跟老闆報告用）。**按工地分區**（筆數多→少、_inbox 殿後）：每塊顯示期間筆數、最後收件、狀態分布徽章、**代表照片縮圖**。**期間下拉**（今天/近7天/近14天/近30天/自訂，預設近7天，改起訖日自動切自訂）。**列印友善**（一鍵 `window.print()`、`@media print` 藏導覽/控制列、區塊不跨頁）。複用 `openRo`/`localDateStr`/`DISPLAYABLE`/`/media`，未動既有頁面，nav 多「報告」。
- **報告頁互動三次來回才定版（溝通復盤）**：縮圖點擊→**頁內 lightbox 放大**（非開新視窗）+ 帶**文字註解**（字級 24px）+ 點背景/Esc/關閉收起。錄音放哪來回 3 次：最終定為**錄音不出現在報告頁**，改成「有錄音的照片右下角標 🎤、放大層『關閉』旁多一顆『🎤 播放錄音』鈕、按了在放大層播」。圖片代表上限 8/工地；純錄音（無照片）紀錄目前報告頁無縮圖、需到詳細頁聽（待評估）。第 2 次失敗主因是我會錯意（已記工作默契：UI 有歧義先複述/示意再動手）。
- smoke-admin 58 → **78**（+20：報告頁分區/篩選/縮圖/排序/lightbox/註解/錄音鈕/列印）。10 支 smoke 全綠（…/inbox 24/command 13/**admin 78**）。

### 下一步
- 後台是獨立程序、**不需 restart bot**；報告頁 `npm run admin` → http://127.0.0.1:3300/report 即可用。
- 純錄音（無照片）紀錄要不要在報告頁補 🎤 縮圖，待使用者決定。
- 匯出報表仍等同學回饋檔名格式；報告頁已先補上「開會/報告」的即時檢視需求。

## 2026-06-14 桌機：分岔整合 + 驗收結算 + 冷啟動補救 + 新增工地極簡化 🟢
桌機開工。先處理跨機分岔（筆電 6/13 那批已 push、桌機 6/11 後台 4 片擱著沒推）：`git pull --rebase` 把後台疊到筆電 6/13 之上、一起 push（程式碼零衝突，docs 四件套手排順序，非 force／非 reset）。接著驗收結算 + 兩塊 bot 體驗強化（**皆要 restart 才生效**）：

- **驗收期結算（5 工作天跑滿）**：6/8–6/12 五個工作天都有資料（14/5/21/5/11 筆）。逐筆驗 15 筆判不出（_inbox）根因 → **零系統誤判**：全是「輸入沒給依據」（5 筆無照片無定位、10 筆純照片無代碼無定位且不在 recent_context 窗內），系統正確進 _inbox 不硬猜——「判不出不硬猜、零誤歸」驗收標準達標。但 6/12 午後一串、6/13 早上一串都是「時段第一批」→ 揭露**冷啟動**痛點（每時段第一張不給依據必判不出）。
- **5-B1 冷啟動批次歸檔**（`PendingInboxStore` + `siteFlow.handleBatchSitePick`）：判不出不再每張洗版；同回報人連續判不出累積、90 秒去抖只送一次選單；點一個工地把這批 _inbox 一次全歸（複用 `applyProjectReassign`）+ 寫 2 小時上下文播種，後續自動歸。選單文案加冷啟動引導。新增 `smoke-inbox.ts`（24）。
- **5-B2 新增工地極簡化**：①`/新增工地 名稱`（免代號）→ `ProjectStore.nextAutoCode` 自動配（前綴自適應現有習慣、找空號補缺、不重用不跳號）；指定碼舊語法相容。②抓定位沿用「先傳一次定位」機制，傳定位+打名稱即自動用定位當中心+設上下文，其餘全自動。新增 `smoke-command.ts`（13）。
- **「一鍵分享定位」按鈕：查證後不做（踩雷）**：Telegram `request_location` 鍵盤按鈕**只在私聊有效、群組無效**（官方隱私限制，不可繞過）。bot 跑在工作群 → 做了也是死的。群組架構下「抓定位」極限＝5-B2 的「手動傳一次、其餘自動」。要一鍵須換架構（私聊／LINE／web），屬規模定案後的決定，不為一顆按鈕現在換——已查 `core/` 無任何 Telegram 依賴，換通道只需加一個 adapter 檔、核心不動。
- 全程 typecheck + **10 支 smoke 全綠**（archive 16 / confirm 15 / site 19 / location 20 / report 14 / voice 16 / append 34 / inbox 24 / command 13 / admin 58）。

### 下一步
- **桌機 restart**：5-B1 + 5-B2 都改 bot，需雙擊 `server/restart-bot.cmd` 才生效（一次重啟兩塊一起套用）。
- 舊的 15 筆 _inbox 不會被 5-B1 追溯處理，可用後台逐筆歸檔或忽略。
- 通道/規模方向（私聊／LINE／web）等使用規模定案再決定。
