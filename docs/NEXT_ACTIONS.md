...
- **錄音存檔**（V0 可做，2026-06-08 補認知）：把 Telegram 語音/音訊訊息**當媒體歸檔**（沿用照片下載→搬檔→DB 管線、`upload_type=voice`、不用改表、不轉文字）。目前語音訊息會被忽略不建檔。語音轉逐字稿（Whisper）仍屬 V1，需先取得公司「送外部 AI」同意。資料夾結構本來就預留 `voices/`。
...
## 待釐清問題
- [ ] **分享 / 給他人獨立開發 + repo 公開私有**（2026-06-08 桌機收工提出，**明天筆電接續**）：repo 目前 **PUBLIC**，待決定是否改 Private；有人要「複製現有基礎、自行另行開發」（做法：`git clone` 後 `rm -rf .git` 重新 init，或 GitHub「Use this template」；對方須自配 bot token / 自己的測試群 / `projects.seed.json`，**不共用** token 與 data）；順道掃 docs 與 git 歷史有無真實案場名/地址（如 `信義豪宅案` 若為真案改代號）。
- [ ] **桌機待辦**：`/新增工地` 新版（座標可選 + 傳位置設中心）已入庫，但**尚未套用到正在跑的背景 bot** —— 需在**桌機**雙擊 `server/restart-bot.cmd`（UAC 同意）才生效，**筆電無法代勞**。
- [ ] **recent_context 2 小時沿用**在實際工作節奏下是否造成誤歸（驗收期觀察；維持現狀，6/13 決定是否縮短時間窗或改強制標代碼，見 DECISIONS 2026-06-08）
