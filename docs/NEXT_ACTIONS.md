...
## 當前進度
**🟢 2026-06-10 晚間再補三塊**：**錄音存檔**（語音/音訊當媒體歸檔 `upload_type=voice`、不轉文字）＋**追加合併**（建檔後 10 分鐘內語音/文字自動併入上一筆、附 🆕 拆成新筆反悔；**封單只認 ✅**，選工地不封單）＋**選單指定工地也記 2 小時上下文**（判不出→選單指定後，後續照片不再每張都問；維持固定 2 小時窗不滑動）。20:09 bot 重啟後**實測 4 筆全驗證通過**（選單選工地後 2 照片+1 錄音全自動歸 C001、✅ 封單後錄音正確開新筆）。實測再加一塊：**定位判不出選單加「➕ 新增工地」**（`loc:_new`＋`PendingLocationStore`：剛傳的定位暫存 10 分鐘，`/新增工地 代碼 名稱` 自動拿它當中心＋設 2 小時上下文）。typecheck + 7 支 smoke 全綠（16/13/19/20/14/16/30）。**⚠️ 「➕ 新增工地」這塊在 20:09 重啟之後才寫——要再跑一次 `restart-bot.cmd` 才生效。**
