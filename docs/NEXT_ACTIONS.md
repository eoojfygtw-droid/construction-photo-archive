...
> ## ⚠️ 桌機開工請先讀（2026-06-13 筆電留）
> **疑似有「後台」工作做了但沒 push、擱在桌機本機(D 槽)。** 筆電端窮舉檢查（所有 branch + origin + C:\ 各 clone + stash）找不到任何後台 commit；桌機在本 repo 最後一次真正 push＝`035f542`（2026-06-10 bot 功能），之後無後台。
> **桌機開工第一件事**：在專案資料夾跑下列,確認後台還在不在、有沒有沒推的 commit——
> ```
> git status                 # 有沒有未提交的後台檔案
> git log origin/main..HEAD  # 列「本機有、還沒 push」的 commit
> git stash list             # 有沒有被 stash 起來的
> ```
> 若列出後台 commit＝「做了沒 push」→ **先 `git pull --rebase` 把筆電 6/13 那幾筆疊進來,再 push**（筆電 6/13 commit 是乾淨疊在 `035f542` 上的純新增,不會動到桌機本機任何檔）。
> **鐵則：絕不 `git push --force`、絕不 `git reset --hard`**——只要不碰這兩個,後台 100% 不會弄丟。確認後台已上 GitHub 後,刪掉這整段警告。
